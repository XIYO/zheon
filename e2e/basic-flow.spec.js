...
test.describe('기본 동작 테스트', () => {
	test('URL 제출 후 새로고침해도 데이터 유지', async ({ page }) => {
		...
		logger.info('[TEST] ✅ 낙관적 업데이트: 항목이 리스트에 추가됨');

		// 서버 응답 대기 (3초)
		await page.waitForTimeout(3000);
		logger.info('[TEST] ⏳ 서버 응답 대기 완료');

		// 3. 새로고침
		logger.info('[TEST] 🔄 페이지 새로고침...');
		await page.reload();
