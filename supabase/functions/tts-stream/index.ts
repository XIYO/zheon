...
interface RequestBody {
  ...
  section: "summary" | "content" | "insights";
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (fetchError) {
      ...
      return new Response(
        JSON.stringify({ error: "데이터 조회 실패", details: fetchError }),
        {
          status: 500,
          headers: {
            "Content-Type": "application/json",
    ...
    if (existingUrl && currentStatus === "completed") {
      ...
      const audioUrl =
        `${SUPABASE_URL}/storage/v1/object/public/tts-audio/${existingUrl}`;
      return new Response(
        JSON.stringify({
          success: true,
          audioUrl,
    ...
    if (currentStatus === "processing") {
      ...
      for (let i = 0; i < maxAttempts; i++) {
        ...
        // @ts-ignore - Dynamic column access
        if (
          pollData && pollData[statusColumn] === "completed" &&
          pollData[audioUrlColumn]
        ) {
          console.log("[TTS] Processing completed by another request");
          // @ts-ignore - Dynamic column access
          const audioUrl =
            `${SUPABASE_URL}/storage/v1/object/public/tts-audio/${
              pollData[audioUrlColumn]
            }`;
          return new Response(
            JSON.stringify({
    ...
    if (updateError) {
      ...
      return new Response(
        JSON.stringify({ error: "락 획득 실패", details: updateError }),
        {
          status: 500,
          headers: {
            "Content-Type": "application/json",
    ...
    if (!updateResult) {
      console.log(
        "[TTS] Failed to acquire lock, another request is processing or completed",
      );
    ...
    // 오디오 데이터 추출
    if (
      !data.candidates || !data.candidates[0]?.content?.parts?.[0]?.inlineData
    ) {
      console.error("[TTS] No audio data in response");
      return new Response(
        JSON.stringify({ error: "오디오 데이터를 찾을 수 없습니다" }),
        {
          status: 500,
          headers: {
            "Content-Type": "application/json",
    ...
    if (uploadError) {
      ...
      return new Response(
        JSON.stringify({ error: "저장소 업로드 실패", details: uploadError }),
        {
          status: 500,
          headers: {
            "Content-Type": "application/json",
    ...
    if (dbUpdateError) {
      ...
      return new Response(
        JSON.stringify({ error: "DB 업데이트 실패", details: dbUpdateError }),
        {
          status: 500,
          headers: {
            "Content-Type": "application/json",
    ...
    // 오디오 URL 반환
    const audioUrl =
      `${SUPABASE_URL}/storage/v1/object/public/tts-audio/${fileName}`;
