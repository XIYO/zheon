...
export const checkDuplicate = RunnableLambda.from(
  async (input: { url: string }) => {
    ...
    if (existingId) {
      console.log(
        `[Check] 🔄 Found existing summary (ID: ${existingId}), timestamps updated`,
      );
      ...
      if (fetchError) {
        console.log(
          `[Check] ⚠️ Failed to fetch existing record: ${fetchError.message}`,
        );
      }
    ...
    console.log(
      `[Check] ✅ No existing summary found, proceeding with new creation`,
    );
