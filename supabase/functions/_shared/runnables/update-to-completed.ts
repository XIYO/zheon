...
export const updateToCompleted = RunnableLambda.from(
	async (input: {
		...
		console.log(`[UpdateCompleted] Updating record ${input.record_id} to completed...`);
		...
		try {
			...
			const { data, error } = await supabase
				...
				.update({
					...
					content: input.insights || input.summary,
			...
			if (error) {
				throw new Error(`Failed to update record to completed: ${error.message}`);
			}

			console.log(`[UpdateCompleted] ✅ Record ${input.record_id} marked as completed`);

			return {
				record_id: data.id,
				saved_at: data.last_modified_at,
				was_duplicate: false
			};
		} catch (error) {
