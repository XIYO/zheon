...
): Promise<ExtractionChainData> {
  console.log(`[Chain 1] 🎬 Extracting subtitles from: ${youtubeUrl}`);

  return extractYouTubeSubtitles(youtubeUrl).then((extraction) => {
...
function saveToDatabase(
  extractionData: ExtractionChainData,
  result: VideoProcessingResult,
): Promise<DatabaseChainData> {
...
function generateSummary(
  databaseData: DatabaseChainData,
  result: VideoProcessingResult,
): Promise<SummaryChainData> {
  ...
  return Promise.resolve().then(() => {
    const chainData: SummaryChainData = {
      ...databaseData,
      summary: `[미구현] ${
        databaseData.transcript.substring(0, 100)
      }... 요약 예정`,
