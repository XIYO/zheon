...
function normalizeYouTubeUrl(url: string): string {
	...
	const isYouTube = youtubePatterns.some((pattern) => pattern.test(parsedUrl.hostname));
		...
		for (const record of allRecords) {
			try {
				...
				if (normalizedData.has(normalizedUrl)) {
					...
					console.log(
						`🔍 Duplicate found: ID ${record.id} duplicates ID ${existing.id} (${normalizedUrl})`
					);
				} else {
				...
				console.error(
					`❌ Error processing record ${record.id}: ${record.youtube_url} - ${error instanceof Error ? error.message : 'Unknown error'}`
				);
			}
		}

		console.log(
			`📊 Analysis complete: ${stats.normalized} need normalization, ${stats.duplicatesFound} duplicates found`
		);
			...
			try {
				...
				if (data.normalizedUrl !== data.originalUrl) {
					...
					if (updateError) {
						...
						updatedRecords++;
						console.log(
							`✅ Updated record ${data.id}: ${data.originalUrl} → ${data.normalizedUrl}`
						);
					}
				}
				...
				if (data.duplicateIds.length > 0) {
					...
					if (deleteError) {
						console.error(
							`❌ Failed to delete duplicates for ${normalizedUrl}: ${deleteError.message}`
						);
					} else {
						removedRecords += data.duplicateIds.length;
						console.log(
							`🗑️ Removed ${data.duplicateIds.length} duplicate records for ${normalizedUrl}: [${data.duplicateIds.join(', ')}]`
						);
					}
				}
			} catch (error) {
				console.error(
					`❌ Error processing ${normalizedUrl}: ${error instanceof Error ? error.message : 'Unknown error'}`
				);
			}
		}
