    ...
    for (const record of allRecords) {
      try {
        ...
        if (needsNormalization) {
          stats.normalized++;
          console.log(
            `🔄 Normalized: ${record.youtube_url} → ${normalizedUrl}`,
          );
        }
        ...
        console.error(
          `❌ Error processing record ${record.id}: ${record.youtube_url} - ${
    ...
    for (const [normalizedUrl, data] of normalizedData) {
      try {
        ...
        if (data.normalizedUrl !== data.originalUrl) {
          ...
          if (updateError) {
            console.error(
              `❌ Failed to update record ${data.id}: ${updateError.message}`,
            );
          } else {
            updatedRecords++;
            console.log(
        ...
        if (data.duplicateIds.length > 0) {
          ...
          if (deleteError) {
            ...
            console.log(
              `🗑️ Removed ${data.duplicateIds.length} duplicate records for ${normalizedUrl}: [${
        ...
        console.error(
          `❌ Error processing ${normalizedUrl}: ${
            error instanceof Error ? error.message : "Unknown error"
          }`,
        );
      }
