...
Deno.serve(async (req: Request): Promise<Response> => {
	...
	try {
		...
		if (!validation.isValid) {
			...
			return corsError(validation.error || 'Validation failed', 'VALIDATION_ERROR', 400);
		}
		...
		console.log(`Processing Gemini request with prompt length: ${prompt.length}`);
		...
		console.log(`Successfully generated response with length: ${responseText.length}`);
