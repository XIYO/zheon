...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  throw new Error("Environment variables required: PUBLIC_SUPABASE_URL and PUBLIC_SUPABASE_ANON_KEY (or SUPABASE_URL and SUPABASE_ANON_KEY)")
}
...
Deno.test("Deployed Edge Function - Complete Environment Variables", async () => {
  ...
  assert(vars.DENO_DEPLOYMENT_ID.includes("iefgdhwmgljjacafqomd"), "Deployment ID should contain project ref")
...
Deno.test("Deployed Edge Function - Function Consistency", async () => {
  ...
  Object.keys(firstData.variables).forEach(key => {
    assertEquals(firstData.variables[key], secondData.variables[key], 
