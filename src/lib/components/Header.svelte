...
<!-- ─────────────────────────── HEADER ─────────────────────────── -->
<header class="sticky top-0 z-50 backdrop-blur-xl bg-surface-50/90 border-b border-surface-200/50 flex items-center justify-between px-6 py-4">
	<div class="flex items-center">
		<a 
			href="/"
			class="group flex items-center space-x-3 text-surface-900 transition-colors hover:text-primary-600">
			<div class="relative">
				<span class="text-3xl font-black tracking-tight gradient-text">展</span>
				<div class="absolute -inset-2 rounded-xl bg-gradient-to-r from-primary-600 to-secondary-600 opacity-0 blur transition-opacity group-hover:opacity-20"></div>
			</div>
			<div class="hidden sm:block">
				<span class="text-xl font-bold tracking-wide">Zheon</span>
				<div class="text-xs text-surface-600 font-medium">YouTube AI 요약</div>
			</div>
		</a>
	</div>
		...
		{:else}
			<div class="flex items-center space-x-4">
				<div class="hidden sm:flex sm:items-center sm:space-x-3">
					<div class="h-8 w-8 rounded-full bg-gradient-to-br from-primary-500 to-secondary-500 flex items-center justify-center text-white text-sm font-semibold">
						{#if page.data.user.user_metadata.avatar_url}
							<img src={page.data.user.user_metadata.avatar_url} alt="User Avatar" class="h-8 w-8 rounded-full object-cover" />
						{:else}
					...
					<div class="text-sm">
						...
						<div class="text-surface-600">환영합니다!</div>
					</div>
				</div>
				<button
...
<dialog
	...
	<div class="card p-6 space-y-6 bg-surface-50 shadow-2xl border border-surface-200">
		<!-- 헤더 -->
...
<dialog
	...
	<div class="card p-6 space-y-6 bg-surface-50 shadow-2xl border border-surface-200">
		<!-- 헤더 -->
		<header class="flex items-center justify-between">
			<h2 class="h3 font-bold text-error-600">로그아웃</h2>
			<form method="DIALOG">
				<button
					aria-label="닫기"
					class="btn variant-ghost-surface btn-sm"
