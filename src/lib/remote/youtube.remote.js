...
async function performChannelVideosSync(channelId) {
	...
	try {
		// 2. YouTube.js로 채널 정보 가져오기
		const channelInfo = await getChannelInfo(channelId);

		// 3. YouTube.js로 비디오 목록 가져오기 (최대 30개)
		const videosData = await getChannelVideos({ channelId, filter: undefined });

		// 4. channels 테이블에 upsert
		upsertChannel({
			channel_id: channelInfo.channelId || channelId,
			channel_name: channelInfo.name,
			channel_handle: channelInfo.handle,
			channel_avatar: channelInfo.thumbnail,
			subscriber_count: channelInfo.subscriberCount,
			description: channelInfo.description,
			video_count: channelInfo.videoCount ? parseInt(channelInfo.videoCount) : undefined,
			channel_data: channelInfo
		});

		// 5. channel_videos 테이블에 upsert (100개씩)
		if (videosData.videos && videosData.videos.length > 0) {
			const videoRows = videosData.videos.map((video) => ({
				channel_id: channelId,
			...
			for (const rowsChunk of chunk(videoRows, 100)) {
				const { error: videosError } = await supabase
					.from('channel_videos')
					.upsert(rowsChunk, { onConflict: 'video_id' });

				if (videosError) throw error(500, videosError.message);
			}
		}

		// 6. 동기화 상태를 completed로 변경
...
/**
 * Form: 채널 비디오 목록 동기화 (버튼 클릭용)
 */
export const syncChannelVideos = form(v.object({
	channelId: v.string()
