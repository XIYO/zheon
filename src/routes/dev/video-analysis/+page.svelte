<script>
  import { analyzeVideoQuality } from '$lib/remote/ai.remote.js';
  ...
  {#if result}
    <section class="space-y-4">
      ...
      <div class="rounded-xl border border-primary-200 bg-primary-50 p-4 dark:border-primary-800 dark:bg-primary-950">
        ...
        <p class="text-sm">
          <strong>Summary ID:</strong> {result.summaryId}
        </p>
      </div>
      ...
      <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <div class="rounded-lg border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
          ...
          <ul class="text-sm space-y-1">
            <li>종합: <strong>{result.analysis.content_quality.overall_score}</strong>/100</li>
            ...
            <li class="pt-2 border-t">
              카테고리: <span class="badge preset-filled">{result.analysis.content_quality.category}</span>
            </li>
            <li>
              대상: <span class="badge preset-filled-secondary">{result.analysis.content_quality.target_audience}</span>
            </li>
          </ul>
        </div>
        ...
        <div class="rounded-lg border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
          ...
          <ul class="text-sm space-y-1">
            <li>종합 여론: <strong>{result.analysis.sentiment.overall_score}</strong>/100</li>
        ...
        <div class="rounded-lg border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
          ...
          <ul class="text-sm space-y-1">
            <li>종합: <strong>{result.analysis.community.overall_score}</strong>/100</li>
      ...
      <div class="space-y-3">
        ...
        {#if result.analysis.summary.key_insights.length > 0}
          <div class="rounded-lg border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
            ...
            <ul class="text-sm space-y-1 list-disc list-inside">
              {#each result.analysis.summary.key_insights as insight}
                <li>{insight}</li>
              {/each}
            </ul>
          </div>
        ...
        {#if result.analysis.summary.recommendations.length > 0}
          <div class="rounded-lg border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
            ...
            <ul class="text-sm space-y-1 list-disc list-inside">
              {#each result.analysis.summary.recommendations as recommendation}
