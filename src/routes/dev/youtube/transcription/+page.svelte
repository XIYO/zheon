<script>
  import { collectTranscript, getTranscriptFromDB } from '$lib/remote/youtube/transcription.remote.js';
...
<main class="container mx-auto px-4 py-8 space-y-6">
  ...
  <section class="rounded-xl border border-surface-200 bg-surface-50 p-6 dark:border-surface-800 dark:bg-surface-900 space-y-4">
    ...
    {#if collectResult}
      <div class="rounded-lg border border-primary-200 bg-primary-50 p-4 dark:border-primary-800 dark:bg-primary-950">
        <p class="text-sm">
          <strong>수집 결과:</strong> {collectResult.message}
        </p>
  ...
  <section class="space-y-4">
    <div class="flex items-center justify-between">
      <h2 class="h3">DB 저장 자막 {transcript ? '(' + transcript.data.segments.length + '개 세그먼트)' : ''}</h2>
      {#if videoId}
    ...
    {#if !transcript}
      <div class="rounded-lg border border-dashed border-surface-200 p-8 text-center text-surface-400 dark:border-surface-700">
      ...
      <div class="space-y-3">
        {#if data.title}
          <div class="rounded-lg border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
            ...
            <p class="text-sm mb-1"><strong>제목:</strong> {data.title}</p>
            {#if data.duration}
              <p class="text-sm"><strong>길이:</strong> {data.duration}초</p>
            {/if}
          </div>
        {/if}
        ...
        <div class="rounded-lg border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
          ...
          <ul class="space-y-2 max-h-96 overflow-y-auto">
            {#each data.segments as segment, idx}
              <li class="border-b border-surface-100 pb-2 last:border-0 dark:border-surface-800">
                <div class="flex items-start gap-3">
                  <span class="text-xs font-mono text-surface-500 dark:text-surface-400 shrink-0 mt-1">
        ...
        <div class="rounded-lg border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
          ...
          <div class="text-sm text-surface-800 dark:text-surface-100 whitespace-pre-wrap max-h-64 overflow-y-auto">
