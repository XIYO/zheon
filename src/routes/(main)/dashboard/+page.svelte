	...
	<div class="relative flex flex-col items-center justify-start animate-fade-in">
		...
		<div class="mb-12 w-full max-w-xl">
			{#if page.form?.message}
				...
				<div in:slide class="mb-6 card {isRateLimit ? 'bg-warning-50/80 border-warning-200/50' : 'bg-error-50/80 border-error-200/50'} backdrop-blur-sm border">
					<div class="flex items-start gap-4 p-6">
						<div class="flex-shrink-0">
							<div class="rounded-full {isRateLimit ? 'bg-warning-100' : 'bg-error-100'} p-2">
								{#if isRateLimit}
						...
						<div class="flex-1">
							<p class="text-sm font-medium {isRateLimit ? 'text-warning-800' : 'text-error-800'}">{page.form.message}</p>
							{#if isRateLimit}
